    ...
    <main class="h-full pb-16 overflow-y-auto">
        <div class="container grid px-6 mx-auto ">
            ...
            <div style='margin-top:40px' class="grid gap-6 mb-8 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2">
                ...
                order_off($conn);
                order_delete($conn);
//                $server_info_select = server_info_select($conn);
//                if (is_array($server_info_select) || is_object($server_info_select)) {
//                    foreach ($server_info_select
//                             as $value):
//                        echo '<div class="shadow-xl min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800 text-xs font-semibold tracking-wide text-left text-gray-500 border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800 ">';
//                        echo '<div class="w-full h-full text-center">';
//                        echo '<div class="flex flex-col justify-between h-full">';
//                        echo '<img class="object-cover mx-auto rounded-full" src="icons/server.png" width="150px" alt="" loading="lazy">';
//                        echo '<p class="mt-3" style="font-size: 14px;font-weight: bold"> <span class="font-bold text-gray-800 dark:text-white"> ' . $value['title'] . ' </span></p>';
//                        echo '<div class="flex items-center justify-between w-full gap-4 mt-8">';
//                        echo '<a id="ts-error" class="py-2 px-4 bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 focus:ring-offset-indigo-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg " href="orders.php?id=' . $value['id'] . '">View server</a>';
//                        echo '</div>';
//                        echo '</div>';
//                        echo '</div>';
//                        echo '</div>';
