    ...
    <div class="w-full h-full text-center">
        <div class="flex flex-col justify-between h-full">
            ...
            <p class="px-6 py-2 text-xs text-gray-600 dark:text-gray-400">
               ip: ' . $row_servers['ip'] . ':' . $row_servers['port'] . '
            </p>';
...
>
    ...
    >
        ...
        if (isset($_GET['delete'])) {
            ...
            $sql_delete_server = "DELETE FROM servers WHERE id='$id_delete_server'";
            $result_delete_server = mysqli_query($conn, $sql_delete_server);
        ...
        if (isset($_GET['on'])) {
            ...
            $sql_on_select = "UPDATE servers SET status='1' WHERE id='$id_on_select'";
        ...
        if(isset($_POST['action']) && $_POST['action'] == 'insert_server_backup') {
            ...
            $insert_server_sql = "INSERT INTO servers (ip,port,username,password,name,status,panel) 
                          VALUES ('$ip_server','$port_server','$username_server','$password_server','$name_server','$status','$panel_server')";
            $result_server_sql = mysqli_query($conn, $insert_server_sql);
        ...
        <form id="insert_server_backup">
            <!-- Modal body -->
                <p class="m-4 dark:text-gray-200 text-gray-700">
                    Notice: Enter the <b style="color: #2ba84a"> numerical ID </b> of the channel in the settings related to backup and
                    be sure to <b style="color: #2ba84a"> administrate </b> the robot in the channel to send the file.
                </p>
            ...
            <label class="block mt-2 text-sm mx-auto" style="width: 97%">
                <select required name="panel"
                        class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                    <option value="/etc/x-ui/x-ui.db">Vaxilu</option>
                    <option value="/etc/x-ui/x-ui.db">Alireza</option>
                    <option value="/etc/x-ui/x-ui.db">Mhsanaei</option>
                    <option value="/etc/x-ui/x-ui.db">FranzKafkaYu</option>
                    <option value="/etc/x-ui/x-ui.db">Hossinasaadi</option>
                    <option value="/etc/x-ui/x-ui.db">HexaSoftwareTech</option>
                </select>
            </label>
